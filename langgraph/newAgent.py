...
from typing import Optional, List, Annotated, TypedDict, Literal, Dict, Any
...
# ==================== 会话存储（内存，生产环境建议用 Redis）====================
session_storage: Dict[str, Dict[str, Any]] = {}

# ==================== 后端图片上传接口配置 ====================

BACKEND_UPLOAD_URL=Config.BACKEND_UPLOAD_URL
BACKEND_EDIT_URL=Config.BACKEND_EDIT_URL
DEFAULT_PRIVATE_SPACE_ID=Config.DEFAULT_PRIVATE_SPACE_ID
...
tools = [analyze_images, retrieve_from_knowledge_base, upload_image, create_pdf_from_story, search_images, delete_images]
...
def call_model(state: AgentState):
    """LLM 节点：调用模型，首次对话时自动注入系统提示引导工具使用"""
    ...
    if not messages or getattr(messages[0], "type", None) != "system":
        ...
        system_prompt = (
            ...
            "⚠️ 重要规则：当用户的问题涉及《西游记》、故事人物、故事情节、神话传说等需要外部知识的内容时，"
...
async def lifespan(app: FastAPI):
    ...
    conn = psycopg.connect(
        os.getenv("DATABASE_URL", "postgresql://langgraph_user:123456@localhost:5432/langgraph_db"),
        autocommit=True,
